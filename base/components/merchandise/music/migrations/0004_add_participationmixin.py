...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Album.submitted_by'
        db.add_column(u'music_album', 'submitted_by',
                      self.gf('django.db.models.fields.related.ForeignKey')(blank=True, related_name='album_submissions', null=True, to=orm['accounts.Editor']),
                      keep_default=False)

        # Adding M2M table for field edited_by on 'Album'
        m2m_table_name = db.shorten_name(u'music_album_edited_by')
        ...
        db.create_unique(m2m_table_name, ['album_id', 'editor_id'])

        # Adding field 'Single.submitted_by'
        db.add_column(u'music_single', 'submitted_by',
                      self.gf('django.db.models.fields.related.ForeignKey')(blank=True, related_name='single_submissions', null=True, to=orm['accounts.Editor']),
                      keep_default=False)

        # Adding M2M table for field edited_by on 'Single'
        m2m_table_name = db.shorten_name(u'music_single_edited_by')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('single', models.ForeignKey(orm[u'music.single'], null=False)),
            ('editor', models.ForeignKey(orm[u'accounts.editor'], null=False))
        ))
        db.create_unique(m2m_table_name, ['single_id', 'editor_id'])

        # Deleting field 'Track.created'
        db.delete_column(u'music_track', 'created')

        # Deleting field 'Track.modified'
        db.delete_column(u'music_track', 'modified')

        # Adding M2M table for field participating_idols on 'Track'
        m2m_table_name = db.shorten_name(u'music_track_participating_idols')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
        ...
        # Adding M2M table for field participating_groups on 'Track'
        m2m_table_name = db.shorten_name(u'music_track_participating_groups')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('track', models.ForeignKey(orm[u'music.track'], null=False)),
    ...
    def backwards(self, orm):
        # Deleting field 'Album.submitted_by'
        db.delete_column(u'music_album', 'submitted_by_id')

        # Removing M2M table for field edited_by on 'Album'
        db.delete_table(db.shorten_name(u'music_album_edited_by'))

        # Deleting field 'Single.submitted_by'
        db.delete_column(u'music_single', 'submitted_by_id')

        # Removing M2M table for field edited_by on 'Single'
        db.delete_table(db.shorten_name(u'music_single_edited_by'))

        # Adding field 'Track.created'
        db.add_column(u'music_track', 'created',
                      self.gf('model_utils.fields.AutoCreatedField')(default=datetime.datetime.now),
                      keep_default=False)

        # Adding field 'Track.modified'
        db.add_column(u'music_track', 'modified',
                      self.gf('model_utils.fields.AutoLastModifiedField')(default=datetime.datetime.now),
                      keep_default=False)

        # Removing M2M table for field participating_idols on 'Track'
        db.delete_table(db.shorten_name(u'music_track_participating_idols'))

        # Removing M2M table for field participating_groups on 'Track'
        db.delete_table(db.shorten_name(u'music_track_participating_groups'))
